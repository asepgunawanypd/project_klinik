...
class M_Kelurahan extends CI_Model {
	...
	var $table = 'tbl_master_kelurahan';
    ...
    public function dataTable($post=''){
        $sql = "SELECT
                tbl_master_kelurahan.id,
                tbl_master_kelurahan.kecamatan_id,
                tbl_master_kelurahan.kelurahan,
                tbl_master_kecamatan.kecamatan
                FROM
                tbl_master_kelurahan
                INNER JOIN tbl_master_kecamatan ON tbl_master_kelurahan.kecamatan_id = tbl_master_kecamatan.id WHERE 1 = 1 ";        
        if(  isset($post['search']) && ! empty($post['search']["value"])  ){
            $search = $this->db->escape_str($post['search']["value"]);            
            $sql .= " AND tbl_master_kelurahan.id LIKE '%". $search ."%' ";
            $sql .= " OR tbl_master_kelurahan.kecamatan_id LIKE '%". $search ."%' ";
            $sql .= " OR tbl_master_kelurahan.kelurahan LIKE '%". $search ."%' ";
        ...
        if( isset($post['order']) && isset($post['order'][0]) ){
            ...
            $sql .= "ORDER BY tbl_master_kelurahan.id ASC";
        }        

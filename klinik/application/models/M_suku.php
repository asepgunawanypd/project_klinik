...
class M_suku extends CI_Model {

	var $table = 'tbl_master_suku';
    ...
    public function dataTable($post=''){
        $sql = "SELECT kd_suku,suku FROM tbl_master_suku WHERE 1 = 1 ";        
        if(  isset($post['search']) && ! empty($post['search']["value"])  ){
            $search = $this->db->escape_str($post['search']["value"]);            
            $sql .= " AND kd_suku LIKE '%". $search ."%' ";
            $sql .= " OR suku LIKE '%". $search ."%' ";
        }   

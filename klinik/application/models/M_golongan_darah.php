...
class M_golongan_darah extends CI_Model {
	...
	var $table = 'tbl_master_golongan_darah';
    ...
    public function dataTable($post=''){
        $sql = "SELECT kd_golongan_darah,golongan_darah FROM tbl_master_golongan_darah AS agm WHERE 1 = 1 ";        
        if(  isset($post['search']) && ! empty($post['search']["value"])  ){
            $search = $this->db->escape_str($post['search']["value"]);            
            $sql .= " AND kd_golongan_darah LIKE '%". $search ."%' ";
            $sql .= " OR golongan_darah LIKE '%". $search ."%' ";
        }   
        ...
        if( isset($post['order']) && isset($post['order'][0]) ){
            ...
            $sql .= "ORDER BY golongan_darah ASC";
        }        
    ...
    public function view($id){
        ...
        $this->db->where('kd_golongan_darah',$id);
        $query = $this->db->get();
        return $query->row();
    }

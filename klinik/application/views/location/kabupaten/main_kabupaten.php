<div class="" role="tabpanel" data-example-id="togglable-tabs">
  <ul id="myTab" class="nav nav-tabs bar_tabs" role="tablist">
    ...
    <li class="active"><a href="<?php echo base_url('Kabupaten'); ?>" id="kab-tab">Kabupaten / Kota</a></li>
    <li class=""><a href="<?php echo base_url('Kecamatan'); ?>" id="kec-tab">Kecamatan</a></li>
  ...
  <div id="myTabContent" class="tab-content">
    <div role="tabpanel" class="tab-pane fade active in" id="tab_content1" aria-labelledby="kab-tab">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>List Kabupaten / Kota <small>Management</small></h2>
          ...
          <div class="x_content">
            <table id="tableData" class="table table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
              <thead>
                <tr>
                  ...
                  <th style="width: 10%;"><div align="center">ID Provinsi</div></th>
            ...
            <div class="clearfix"></div>
              <div class="x_content">
                  ...
                  <a href="<?php echo base_url('Kabupaten/printpdf'); ?>" target="_blank" class="btn btn-info" title="Print"><i class="glyphicon glyphicon-print"></i> Print</a>
                  <!--?php } if($access->user_ex != 0){ ?-->
                  <a href="<?php echo base_url('Kabupaten/export'); ?>" class="btn btn-info"><i class="glyphicon glyphicon glyphicon-floppy-save"></i> Export Excel</a>
                  <!--?php } ?-->
              </div>
          </div>
        </div>
...
<script type="text/javascript">
    ...
    var table = $('#tableData').DataTable({ 
        ...
        ajax: {
            url: "<?php echo base_url('Kabupaten/dataTable')?>",
            type: "POST"
        },
        language: {
          infoFiltered: " "
        ...
        columns: [
            ...
            { "data": "provinsi_id",
               fnCreatedCell: function (nTd, sData, oData, iRow, iCol) {
                $(nTd).html('<div align="center">'+oData.provinsi_id+'</div>');
               }
            },
            { "data": "id",
               fnCreatedCell: function (nTd, sData, oData, iRow, iCol) {
    ...
    $('#view').click( function (){
        ...
        if( ! row ){ swal("Warning", "No data selected", "info"); }
          ...
          $.post("<?php echo base_url('Kabupaten/view'); ?>", { id:row.id  }, function(data) {
            var obj = JSON.parse(data);
            $('#modal_view').modal({show:true});
            $('#v_id').text(': '+obj.id);
            $('#v_provinsi_id').text(': '+obj.provinsi_id);
            $('#v_provinsi').text(': '+obj.provinsi);
	...
	$( "#id" ).on('keyup', function( event ) {
		...
		var id = $(this).val();
		$.post("<?php echo base_url('Kabupaten/filename_exists'); ?>", { id:id  }, function(data) {   
			if (data == 'error') {
	...
	$('#form').submit(function(e){
			...
			var cek;
			if(save_method == 'add'){
			   url = "<?php echo site_url('Kabupaten/add')?>";
			   cek = globalVar;
			}else{
			   url = "<?php echo site_url('Kabupaten/update')?>";
			   cek = 'ok';
			}
			if(cek == 'error'){
		...
		$('#edit').click(function(){
            ...
            if( ! data ){ swal("No data selected", "", "info"); }
				...
				$.post("<?php echo base_url('Kabupaten/edit_view'); ?>", { id:data.id  }, function(data) {
					var obj = JSON.parse(data);
					$('[name="id"]').val(obj.id);
					$('[name="id"]').attr('readonly', true);
					$('[name="kabupaten"]').val(obj.kabupaten);
					$('[name="provinsi_id"]').val(obj.provinsi_id);
          $('[name="created_by"]').val(obj.created_by);
          $('[name="created_at"]').val(obj.created_at);
					$('#modal_crud').modal({show:true});
					$('.modal-title').text('Edit Kabupaten / Kota');
					window.id = obj.id;
				});
			}
		});
		$('#delete').click(function(){
            ...
            if( ! data ){ swal("No data selected", "", "info"); }
               ...
               swal({
                 ...
                 $.ajax({
                   url : "<?php echo site_url('Kabupaten/delete')?>/"+data.id,
                   type: "POST",
                   success: function(data){               

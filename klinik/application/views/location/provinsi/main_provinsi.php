<div class="" role="tabpanel" data-example-id="togglable-tabs">
  <ul id="myTab" class="nav nav-tabs bar_tabs" role="tablist">
    <li class="active"><a href="<?php echo base_url('Provinsi'); ?>" id="pro-tab">Provinsi</a></li>
  ...
  <div id="myTabContent" class="tab-content">
    <div role="tabpanel" class="tab-pane fade active in" id="tab_content1" aria-labelledby="pro-tab">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>List Provinsi<small>Management</small></h2>
          ...
          <div class="x_content">
            ...
            <div class="clearfix"></div>
              <div class="x_content">
                  ...
                  <?php } if($access->location_u != 0){ ?>
                  <button id="edit" type="button" class="btn btn-warning"><i class="glyphicon glyphicon-pencil"></i> Edit</button>
                  <?php } if($access->location_d != 0){ ?>
                  <button id="delete" type="button" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> Delete</button>
                  <?php } if($access->location_p != 0){ ?>
                  <a href="<?php echo base_url('Provinsi/printpdf'); ?>" target="_blank" class="btn btn-info" title="Print"><i class="glyphicon glyphicon-print"></i> Print</a>
                  <?php } if($access->location_ex != 0){ ?>
                  <a href="<?php echo base_url('Provinsi/export'); ?>" class="btn btn-info"><i class="glyphicon glyphicon glyphicon-floppy-save"></i> Export Excel</a>
                  <?php } ?>
              </div>
          </div>
        </div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){ 
		...
		var table = $('#tableData').DataTable({ 
			...
			ajax: {
				url: "<?php echo base_url('Provinsi/dataTable')?>",
				type: "POST"
			},
			language: {
			  infoFiltered: " "
		...
		$('#tableData tbody').on( 'click', 'tr', function () {
		 ...
		 $('#view').click( function (){
			...
			if( ! row ){ swal("Warning", "No data selected", "info"); }
			  ...
			  $.post("<?php echo base_url('Provinsi/view'); ?>", { id:row.id  }, function(data) {
		...
		$( "#id" ).on('keyup', function( event ) {
			...
			var id = $(this).val();
			$.post("<?php echo base_url('Provinsi/filename_exists'); ?>", { id:id  }, function(data) {   
				if (data == 'error') {
		...
		$('#form').submit(function(e){
			...
			var cek;
			if(save_method == 'add'){
			   url = "<?php echo site_url('Provinsi/add')?>";
			   cek = globalVar;
			}else{
			   url = "<?php echo site_url('Provinsi/update')?>";
			   cek = 'ok';
			}
			if(cek == 'error'){
		...
		$('#edit').click(function(){
            ...
            if( ! data ){ swal("No data selected", "", "info"); }
				...
				$.post("<?php echo base_url('Provinsi/edit_view'); ?>", { id:data.id  }, function(data) {
					var obj = JSON.parse(data);
					$('[name="id"]').val(obj.id);
					$('[name="id"]').attr('readonly', true);
					$('[name="provinsi"]').val(obj.provinsi);
		...
		$('#delete').click(function(){
            ...
            if( ! data ){ swal("No data selected", "", "info"); }
               ...
               swal({
                 ...
                 $.ajax({
                   url : "<?php echo site_url('Provinsi/delete')?>/"+data.id,
                   type: "POST",
                   success: function(data){               

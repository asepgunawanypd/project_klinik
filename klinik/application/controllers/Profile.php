...
class Profile extends AUTH_Controller {
	...
	public function update() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$this->updateProfil();
				$this->session->set_flashdata('msg', 'Success Data Saved');
				redirect('Profile');
			} else {
				$this->session->set_flashdata('msg','No data saved');
				redirect('Profile');
			}
		} else {
		...
		if ($this->form_validation->run() == TRUE) {
			if (md5($this->input->post('passLama')) == $this->userdata->password) {
				if ($this->input->post('passBaru') != $this->input->post('passKonf')) {
					$this->session->set_flashdata('msg', 'Password Baru dan Konfirmasi Password harus sama');
					redirect('Profile');
				} else {
					$data = [
					...
					if ($result > 0) {
						$this->updateProfil();
						$this->session->set_flashdata('msg', ' Success Password Saved');
						redirect('Profile');
					} else {
				...
				$this->session->set_flashdata('msg', 'Password False !');
				redirect('Profile');
			}
		} else {
			$this->session->set_flashdata('msg',validation_errors());
